...
class GestureController:
    ...
    def _posture_to_mode(self, posture):
        return {
            ...
            "THREE_FINGERS": 3,
            "FOUR_FINGERS":  4,
            # Static gestures that all belong to mode 1
            "FIST":          1,
            "OPEN_PALM":     1,
            "THUMB_UP":      1,
        }.get(posture, 0)
    ...
    def _update_mode(self, posture, now):
        ...
        if target == 0:
            # Unrecognised posture — freeze current mode, don't start countdown
            self.mode_candidate   = 0
            self.mode_hold_frames = 0
            return
...
def draw_overlay(frame, controller: GestureController, posture, action, show_help):
    ...
    for i, entry in enumerate(controller.action_log):
        cv2.putText(frame, entry, (10, 237 + i * 16), FONT, 0.32, C_WHITE, 1)
